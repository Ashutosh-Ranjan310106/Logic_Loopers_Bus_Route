...
class UserService: 
    ...
    def create_user(name, email, gender, phone_number, password, connection, cursor):
        try:
            ...
            connection.rollback()
            log_error(f"create user {name, email, gender, phone_number, password}", e)
            return -1
    ...
    def login_user(email, phone_number, password, user_ip, connection, cursor):
        try:
            ...
            log_error(f"login user {email, phone_number, password, user_ip}", e)
            connection.rollback()
            return -3
    ...
    def logout_user(user_ip, connection, cursor):
        try:
            ...
            log_error(f"logout user {user_ip}", e)
            return -2
    ...
    def getfare(starting_stop_number, ending_stop_number, category, bus_number, connection, cursor):
        try:
            ...
            log_error(f"get fare {starting_stop_number, ending_stop_number, category, bus_number, query}", e)
            return -2
    ...
    def get_user_tickets(user_ip, connection, cursor):
        try:
            ...
            log_error(f"get user tickets {user_ip}", e)
            return -2
    ...
    def book_online_tickets(user_ip, route_id, starting_stop_number, ending_stop_number, price, gender, category, connection, cursor):
        try:
            ...
            log_error(f'book onine ticket {user_ip, route_id, starting_stop_number, ending_stop_number, price, gender, category}', e)
            return -2
