    ...
    def process_path(paths, connection, cursor):
        ...
        for path in paths:
            ...
            for sub_path in path:
                ...
                sub_path['ac_fare'] = 10
                sub_path['non_ac_fare'] = 5
                for stop in sub_path['stop_list']:
                    fare_stage = stop["Fare_stage"]
                    

                    # Accumulate fares for the path
                    sub_path['ac_fare'] += abs(fare_stage - current_fare_stage) * 5
                    sub_path['non_ac_fare'] += abs(fare_stage - current_fare_stage) * 5
                    current_fare_stage = fare_stage

                total_ac_fare += sub_path['ac_fare']
                total_non_ac_fare += sub_path['non_ac_fare']
